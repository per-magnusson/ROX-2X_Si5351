...
typedef enum {
  ...
  SI5351_DISABLE_HIGH = 1,
  SI5351_DISABLE_HIZ = 2,
  SI5351_DISABLE_NEVER = 3
} si5351Disable_t;
...
typedef struct {
  ...
  si5351Drive_t drive[3];          //!< Output drive strength
  si5351RDiv_t rdiv[3];            //!< Rdiv values for each output
  bool invert[3];                  //!< Wheter to invert the respective output
} si5351Config_t;
...
public:
  ...
  err_t resetPLL(si5351PLL_t pll);       //!< @return ERROR_NONE
  err_t setClockBuilderData(void);       //!< @return ERROR_NONE
  err_t setupPLL(si5351PLL_t pll, uint8_t mult, uint32_t num,
                 uint32_t denom,bool doReset = true, 
                 bool autoIntMode = true);  //!< @return ERROR_NONE
  err_t setupPLLInt(si5351PLL_t pll, uint8_t mult, bool doReset = true); //!< @return ERROR_NONE
  err_t setupMultisynth(uint8_t output, si5351PLL_t pllSource, uint32_t div,
                        uint32_t num, uint32_t denom, bool autoIntMode = true); //!< @return ERROR_NONE
  ...
  err_t enableOutput(uint8_t output, bool enable);
  err_t invertOutput(uint8_t output, bool invert);
  err_t powerDownOutput(uint8_t output, bool pdown);
  err_t outputDrive(uint8_t output, si5351Drive_t drive);
  err_t disableState(uint8_t output, si5351Disable_t dstate);
